...
;
export function BTreeVisualizer(conf) {
    let depth = 0, width = 0, height = 0, svgTree = tag("g", "", {
        class: "svg-tree",
...
function visualizeNode(node, svg, minx, maxx, y, conf) {
    if (node == undefined)
        return 0;
    let halfWidth = conf.WIDTH / 2 | 0, centerX = minx + (maxx - minx) / 2 | 0, centerY = y + halfWidth, circleRadius = conf.WIDTH / 2 | 0, cl = conf.nodeClass ? conf.nodeClass(node) : "", nextYStart = y + conf.HEIGHT, svgNodeX = centerX - circleRadius, svgNodeY = centerY - circleRadius, svgNode = tag("g", "", {
        class: "svg-node " + cl,
        transform: `translate(${svgNodeX} ${svgNodeY})`
    }), svgCircle = tag("circle", "", {
        cx: circleRadius,
        cy: circleRadius,
        r: circleRadius
    }), svgText = tag("text", "", {
    ...
    if (!node.isLeaf) {
        ...
        if (node.left) {
            ...
            svg.appendChild(tag("line", "", lineAttrs(centerX, centerY, childrenX, childrenY, circleRadius)));
        }
        if (node.right) {
