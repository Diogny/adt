...
export class TreeNode extends ValueNode {
...
export class BaseTree {
    ...
    *levelOrderEnumerator(node) {
        let queue = new Queue(), maxLevel = 0;
        !node && (node = this.root);
        if (node) {
...
export class Tree extends BaseTree {
     ...
     */
    find(value) {
        let queue = new Queue(), node = this.root;
        if (node) {
            queue.enqueue(node);
            while (!queue.empty) {
                node = queue.dequeue();
